...
if (!isset($_SESSION['coins'])) {
    ...
    if (!$user) {
        ...
        // Vérifier si la colonne coins existe dans la table users
        $checkColumn = $pdo->query("SHOW COLUMNS FROM users LIKE 'coins'");
        if ($checkColumn->rowCount() === 0) { // Si la colonne coins n'existe pas, alors on l'ajoute
        ...
        <div class="shop-header">
            ...
            <div class="pack-grid">
                <?php while ($card = $cardsResult->fetch()): ?>
                    <div class="pack-card" data-card-id="<?php echo $card['id']; ?>"> <!-- ID de la carte -->
                        ...
                        <div class="pack-content">
                            <div class="card-content">
                                <h3 class="card-title"><?php echo htmlspecialchars($card['name']); ?></h3> <!-- Nom de la carte -->
...
function buyCard(cardId) {
    if (confirm('Voulez-vous vraiment acheter cette carte ?')) {
        // Désactiver tous les boutons d'achat pour éviter les clics multiples (fesait crash le site)
        const buyButtons = document.querySelectorAll('.btn-buy');
        buyButtons.forEach(btn => btn.disabled = true);
        
        // Envoyer la requête à l'API pour acheter la carte
        fetch('api/buy_card.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ card_id: cardId }) // Envoyer l'ID de la carte
        })
        .then(response => {
            if (!response.ok) {
